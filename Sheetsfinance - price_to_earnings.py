...
PROGRESS_PATH = Path("0_ingestion") / "price_to_earnings_progress.json"
RUN_STATE_PATH = Path("0_ingestion") / "price_to_earnings_last_run.json"

DEFAULT_SPREADSHEET_ID = "1h6hyiBGSJvXNLAh_7PTTo_4Qc3f_KJg8JuFXT2zY1Gk"
...
def metric_formula(row: int, header_cells: dict[str, str]) -> str:
    return f'=SF({ticker_ref(row, header_cells)},{excel_quote("ratios")},{excel_quote("priceEarningsRatio")},{excel_quote("ttm")})'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh price_to_earnings in tickers cell-by-cell, sync it to tickers_output, and export tickerlist.xlsx.")
    ...
    parser.add_argument("--poll-seconds", type=float, default=float(os.getenv("INGESTION_PRICE_TO_EARNINGS_POLL_SECONDS", str(DEFAULT_POLL_SECONDS))))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.getenv("INGESTION_PRICE_TO_EARNINGS_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT_SECONDS))))
    parser.add_argument("--min-wait-seconds", type=float, default=float(os.getenv("INGESTION_PRICE_TO_EARNINGS_MIN_WAIT_SECONDS", str(DEFAULT_MIN_WAIT_SECONDS))))
    parser.add_argument("--stable-reads", type=int, default=int(os.getenv("INGESTION_PRICE_TO_EARNINGS_STABLE_READS", str(DEFAULT_STABLE_READS))))
    parser.add_argument("--speedrun-wait-seconds", type=float, default=float(os.getenv("INGESTION_PRICE_TO_EARNINGS_SPEEDRUN_WAIT_SECONDS", str(DEFAULT_SPEEDRUN_WAIT_SECONDS))))
    parser.add_argument("--max-refresh-passes", type=int, default=int(os.getenv("INGESTION_PRICE_TO_EARNINGS_MAX_REFRESH_PASSES", str(DEFAULT_MAX_REFRESH_PASSES))))
