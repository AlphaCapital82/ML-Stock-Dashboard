...
PROGRESS_PATH = Path("0_ingestion") / "earnings_surprise_progress.json"
RUN_STATE_PATH = Path("0_ingestion") / "earnings_surprise_last_run.json"

DEFAULT_SPREADSHEET_ID = "1PHYPBjp-sbLYiep5K4b9tTIRj877mwfPIRcQvc82M3c"
...
ACTUAL_EPS_HEADER = "actual eps"
ESTIMATED_EPS_HEADER = "estimated eps"
...
def actual_eps_formula(row: int, header_cells: dict[str, str]) -> str:
    return (
        f'=IFERROR(INDEX(SF({ticker_ref(row, header_cells)},{excel_quote("earnings")},'
        f'{excel_quote("eps")},{excel_quote("ttm")},{excel_quote("NH")}),1,1),{excel_quote("")})'
    )


def estimated_eps_formula(row: int, header_cells: dict[str, str]) -> str:
    return (
        f'=IFERROR(INDEX(SF({ticker_ref(row, header_cells)},{excel_quote("earnings")},'
        f'{excel_quote("epsEstimated")},{excel_quote("ttm")},{excel_quote("NH")}),1,1),{excel_quote("")})'
    )
...
def run_metric_refresh(
    ...
    actual_eps_column = require_header_cell(header_cells, ACTUAL_EPS_HEADER, source_sheet_name)
    estimated_eps_column = require_header_cell(header_cells, ESTIMATED_EPS_HEADER, source_sheet_name)
    ...
    print(
        f"Starting {METRIC_HEADER}: worksheet={source_sheet_name}, rows 2-{last_row}, "
        f"refreshing {ACTUAL_EPS_HEADER!r} and {ESTIMATED_EPS_HEADER!r}, output={output_sheet_name}",
        flush=True,
    )
    ...
    for pass_number in range(1, max_refresh_passes + 1):
        ...
        formula_updates: list[tuple[str, list[list[str | int | float]]]] = []
        for row_number, _, _ in pending_rows:
            formula_updates.extend(
                [
                    (f"{source_sheet_name}!{actual_eps_column}{row_number}", [[actual_eps_formula(row_number, header_cells)]]),
                    (f"{source_sheet_name}!{estimated_eps_column}{row_number}", [[estimated_eps_formula(row_number, header_cells)]]),
                ]
            )
        if pass_number == 1:
            print(f"Refresh pass {pass_number}/{max_refresh_passes}: refreshing {len(pending_rows)} ticker rows.", flush=True)
        else:
            print(f"Refresh pass {pass_number}/{max_refresh_passes}: refreshing {len(pending_rows)} error/not-ready rows.", flush=True)
        ...
        actual_values = get_google_range_values(
            service,
            spreadsheet_id,
            f"{source_sheet_name}!{actual_eps_column}{read_start_row}:{actual_eps_column}{read_end_row}",
            value_render_option="UNFORMATTED_VALUE",
        )
        estimated_values = get_google_range_values(
            service,
            spreadsheet_id,
            f"{source_sheet_name}!{estimated_eps_column}{read_start_row}:{estimated_eps_column}{read_end_row}",
            value_render_option="UNFORMATTED_VALUE",
        )
        metric_values = get_google_range_values(
            service,
            spreadsheet_id,
        ...
        for row_number, ticker_text, previous_raw_value in pending_rows:
            ...
            raw_actual = (
                actual_values[retry_value_index][0]
                if retry_value_index < len(actual_values) and actual_values[retry_value_index]
                else ""
            )
            raw_estimated = (
                estimated_values[retry_value_index][0]
                if retry_value_index < len(estimated_values) and estimated_values[retry_value_index]
                else ""
            )
            raw_metric = (
                metric_values[retry_value_index][0]
                if retry_value_index < len(metric_values) and metric_values[retry_value_index]
                else ""
            )
            actual_number = coerce_valid_number(raw_actual)
            estimated_number = coerce_valid_number(raw_estimated)
            metric_number = coerce_valid_number(raw_metric)
            if actual_number is None or estimated_number is None or metric_number is None:
                latest_raw = (
                    f"{ACTUAL_EPS_HEADER}={short_sheet_value(raw_actual)}, "
                    f"{ESTIMATED_EPS_HEADER}={short_sheet_value(raw_estimated)}, "
                    f"{METRIC_HEADER}={short_sheet_value(raw_metric)}"
                )
                next_pending_rows.append((row_number, ticker_text, latest_raw or previous_raw_value))
                continue
            output_updates.append((f"{output_sheet_name}!{output_column}{row_number}", [[metric_number]]))
            refreshed_rows += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh earnings_surprise in tickers cell-by-cell, sync it to tickers_output, and export tickerlist.xlsx.")
    ...
    parser.add_argument("--poll-seconds", type=float, default=float(os.getenv("INGESTION_EARNINGS_SURPRISE_POLL_SECONDS", str(DEFAULT_POLL_SECONDS))))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.getenv("INGESTION_EARNINGS_SURPRISE_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT_SECONDS))))
    parser.add_argument("--min-wait-seconds", type=float, default=float(os.getenv("INGESTION_EARNINGS_SURPRISE_MIN_WAIT_SECONDS", str(DEFAULT_MIN_WAIT_SECONDS))))
    parser.add_argument("--stable-reads", type=int, default=int(os.getenv("INGESTION_EARNINGS_SURPRISE_STABLE_READS", str(DEFAULT_STABLE_READS))))
    parser.add_argument("--speedrun-wait-seconds", type=float, default=float(os.getenv("INGESTION_EARNINGS_SURPRISE_SPEEDRUN_WAIT_SECONDS", str(DEFAULT_SPEEDRUN_WAIT_SECONDS))))
    parser.add_argument("--max-refresh-passes", type=int, default=int(os.getenv("INGESTION_EARNINGS_SURPRISE_MAX_REFRESH_PASSES", str(DEFAULT_MAX_REFRESH_PASSES))))
