...
PROGRESS_PATH = Path("0_ingestion") / "debt_to_assets_progress.json"
RUN_STATE_PATH = Path("0_ingestion") / "debt_to_assets_last_run.json"

DEFAULT_SPREADSHEET_ID = "1z_j9QHFcw5m47GQsduOIwEabPitRHkGcM7u_aQGmz1g"
...
def metric_formula(row: int, header_cells: dict[str, str]) -> str:
    return f'=SF({ticker_ref(row, header_cells)},{excel_quote("ratios")},{excel_quote("debtToAssetsRatio")},{excel_quote("ttm")})'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh debt_to_assets in tickers cell-by-cell, sync it to tickers_output, and export tickerlist.xlsx.")
    ...
    parser.add_argument("--poll-seconds", type=float, default=float(os.getenv("INGESTION_DEBT_TO_ASSETS_POLL_SECONDS", str(DEFAULT_POLL_SECONDS))))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.getenv("INGESTION_DEBT_TO_ASSETS_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT_SECONDS))))
    parser.add_argument("--min-wait-seconds", type=float, default=float(os.getenv("INGESTION_DEBT_TO_ASSETS_MIN_WAIT_SECONDS", str(DEFAULT_MIN_WAIT_SECONDS))))
    parser.add_argument("--stable-reads", type=int, default=int(os.getenv("INGESTION_DEBT_TO_ASSETS_STABLE_READS", str(DEFAULT_STABLE_READS))))
    parser.add_argument("--speedrun-wait-seconds", type=float, default=float(os.getenv("INGESTION_DEBT_TO_ASSETS_SPEEDRUN_WAIT_SECONDS", str(DEFAULT_SPEEDRUN_WAIT_SECONDS))))
    parser.add_argument("--max-refresh-passes", type=int, default=int(os.getenv("INGESTION_DEBT_TO_ASSETS_MAX_REFRESH_PASSES", str(DEFAULT_MAX_REFRESH_PASSES))))
