...
PROGRESS_PATH = Path("0_ingestion") / "ev_to_ebitda_progress.json"
RUN_STATE_PATH = Path("0_ingestion") / "ev_to_ebitda_last_run.json"

DEFAULT_SPREADSHEET_ID = "1mfIcxXzMmJuD1ljsOOtkOO74qoChS-SMHbO-2561hpU"
...
def metric_formula(row: int, header_cells: dict[str, str]) -> str:
    return f'=SF({ticker_ref(row, header_cells)},{excel_quote("ratios")},{excel_quote("evToEBITDA")},{excel_quote("ttm")})'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh ev_to_ebitda in tickers cell-by-cell, sync it to tickers_output, and export tickerlist.xlsx.")
    ...
    parser.add_argument("--poll-seconds", type=float, default=float(os.getenv("INGESTION_EV_TO_EBITDA_POLL_SECONDS", str(DEFAULT_POLL_SECONDS))))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.getenv("INGESTION_EV_TO_EBITDA_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT_SECONDS))))
    parser.add_argument("--min-wait-seconds", type=float, default=float(os.getenv("INGESTION_EV_TO_EBITDA_MIN_WAIT_SECONDS", str(DEFAULT_MIN_WAIT_SECONDS))))
    parser.add_argument("--stable-reads", type=int, default=int(os.getenv("INGESTION_EV_TO_EBITDA_STABLE_READS", str(DEFAULT_STABLE_READS))))
    parser.add_argument("--speedrun-wait-seconds", type=float, default=float(os.getenv("INGESTION_EV_TO_EBITDA_SPEEDRUN_WAIT_SECONDS", str(DEFAULT_SPEEDRUN_WAIT_SECONDS))))
    parser.add_argument("--max-refresh-passes", type=int, default=int(os.getenv("INGESTION_EV_TO_EBITDA_MAX_REFRESH_PASSES", str(DEFAULT_MAX_REFRESH_PASSES))))
