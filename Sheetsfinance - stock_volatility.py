...
PROGRESS_PATH = Path("0_ingestion") / "stock_volatility_progress.json"
RUN_STATE_PATH = Path("0_ingestion") / "stock_volatility_last_run.json"

DEFAULT_SPREADSHEET_ID = "11G8OO8_XhHCh1-nWY7rGTMMoPjTb0svMiYCxBQb9YWk"
...
def metric_formula(row: int, header_cells: dict[str, str]) -> str:
    return (
        f'=IFERROR(STDEV.P(SF_TIMESERIES({ticker_ref(row, header_cells)},TODAY()-90,TODAY(),'
        f'{excel_quote("")},{excel_quote("changePercent")},{excel_quote("NH")}))/100,{excel_quote("")})'
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh stock_volatility in tickers cell-by-cell, sync it to tickers_output, and export tickerlist.xlsx.")
    ...
    parser.add_argument("--poll-seconds", type=float, default=float(os.getenv("INGESTION_STOCK_VOLATILITY_POLL_SECONDS", str(DEFAULT_POLL_SECONDS))))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.getenv("INGESTION_STOCK_VOLATILITY_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT_SECONDS))))
    parser.add_argument("--min-wait-seconds", type=float, default=float(os.getenv("INGESTION_STOCK_VOLATILITY_MIN_WAIT_SECONDS", str(DEFAULT_MIN_WAIT_SECONDS))))
    parser.add_argument("--stable-reads", type=int, default=int(os.getenv("INGESTION_STOCK_VOLATILITY_STABLE_READS", str(DEFAULT_STABLE_READS))))
    parser.add_argument("--speedrun-wait-seconds", type=float, default=float(os.getenv("INGESTION_STOCK_VOLATILITY_SPEEDRUN_WAIT_SECONDS", str(DEFAULT_SPEEDRUN_WAIT_SECONDS))))
    parser.add_argument("--max-refresh-passes", type=int, default=int(os.getenv("INGESTION_STOCK_VOLATILITY_MAX_REFRESH_PASSES", str(DEFAULT_MAX_REFRESH_PASSES))))
